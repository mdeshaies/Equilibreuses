    ...
    , ui(new Ui::WidgetUSB202)
{
    ...
    erreur = laCarte.ulDConfigBit(BP1,DD_INPUT);
    if(erreur != ERR_NO_ERROR)
        qDebug() << "Config " << BP1 << " Code Erreur : " << erreur;
    erreur = laCarte.ulDConfigBit(BP2,DD_INPUT);
    if(erreur != ERR_NO_ERROR)
        qDebug() << "Config " << BP2 << " Code Erreur : " << erreur;
    erreur = laCarte.ulDConfigBit(BP3,DD_INPUT);
    if(erreur != ERR_NO_ERROR)
        qDebug() << "Config " << BP3 << " Code Erreur : " << erreur;
    erreur = laCarte.ulDConfigBit(BP4,DD_INPUT);
    if(erreur != ERR_NO_ERROR)
        qDebug() << "Config " << BP4 << " Code Erreur : " << erreur;
